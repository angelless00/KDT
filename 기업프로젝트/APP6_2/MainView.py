...
# 디자인 qrc파일
sys.path.append(r'C:\KDT\기업프로젝트\APP6_2\resources') 
...
class MainView(QMainWindow, form_class):
    def __init__(self):
        ...
        self.signal.loglist_signal.connect(self.update_log_list) # 추가
    ...
    def update_log_list(self, combined_message):
        message, reck = combined_message.rsplit('|', 1)  # 문자열을 나누어 message와 reck 추출
        reck = int(reck)    # showlist 호출

        self.showlist(message)
         # 팝업 알림 표시
        self.show_popup(reck)


    def show_popup(self, reck):
        alert = QMessageBox()
        if reck == 0:
                alert.setText("헬멧 미착용 경고!")
                alert.setIcon(QMessageBox.Warning)
        elif reck == 1:
                alert.setText("사고 발생 경고!")
                alert.setIcon(QMessageBox.Warning)
        elif reck == 2:
                alert.setText("위험 경고! 즉시 확인하세요!")

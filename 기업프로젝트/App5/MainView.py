...
class MainView(QMainWindow, form_class):
    ...
    def load_video(self):
        ...
        if self.video_file:
            ...
            ###
            self.video_thread.accident_signal.connect(self.accident_fun)
            self.cntTh=20           # 프레임 몇번 참아줄껀데?
            self.car_cnt=0
            self.helmet_cnt=0
...
#####
    ...
    def make_log(self,rack,case,video_path):
        ...
        with open(f'log/{now}.txt',mode='a+') as f:
            f.write(f'{filename} {rack} case_{case} {now} {video_path}\n')
    ...
    def no_helmet(self,DF):
        if len(DF):
            if len(DF[DF['class']==1]):
                ...
                self.car_cnt=max(self.helmet_cnt-1,0)
